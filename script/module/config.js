...
const THEME_PATHNAME = '/appearance/themes/Tsundoku Dark';

export var config = {
    token: '', // API token, 无需填写
    custom: {
        // 自定义配置
        path: '/appearance/themes/Tsundoku Darkn/custom.json', // 自定义配置文件路径
    },
    theme: {
        regs: {
            // 正则表达式
            ...
            elements: {
                ...
                img: {
                    ...
                    style: {
                        ...
                        innerHTML:
                            'img:not(.emoji, .thumbnailImage), div.thumbnailSelectionRing {filter: invert(100%);}div.protyle-background__icon>img,span.b3-list-item__icon>img {filter: none;}', // 样式标签内容
                    },
                },
                viewer: {
                    // PDF 预览反色
...
export var custom = {
    theme: {
        toolbar: {
            [config.theme.toolbar.more.id]: { default: false }, // 工具栏是否展开
            [config.theme.location.record.toolbar.id]: { default: false }, // 当前浏览位置
            [config.theme.menu.block.toolbar.id]: { default: true }, // 块功能增强
